...
import { DataProps, EducationProps, InformationProps, ProjectProps, WorkExperienceProps } from "@/types";
...
export const getStaticProps = async () => {
  ...
  const educationWithData = objectData.education.map(
    async (item: WorkExperienceProps) => {
      return getImgSrc({
        section: "education",
        item: await getMd({ section: "education", item }),
      });
    },
  );
  ...
  return {
    props: {
      ...
      education: await Promise.all(educationWithData),
    },
  };
};
...
const getMd = async ({
  ...
  section: string;
  item: InformationProps | ProjectProps | WorkExperienceProps | EducationProps;
}) => {
  try {
